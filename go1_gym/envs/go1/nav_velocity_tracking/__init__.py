...
from go1_gym.envs.base.navigator import LeggedRobot
...
class VelocityTrackingEasyEnv(LeggedRobot):
    ...
    def step(self, actions):
        ...
        self.obs_buf, self.rew_buf, self.reset_buf, self.extras = super().step(actions)
        ...
        # phase2
        self.extras.update({
            "loco_obs": self.obs_buf,
        })
        
        return self.nav_obs_buf, self.rew_buf, self.reset_buf, self.extras


    def reset(self):
        self.reset_idx(torch.arange(self.num_envs, device=self.device))
        obs, _, _, _ = self.step(torch.zeros(self.num_envs, self.num_nav_actions, device=self.device, requires_grad=False))
        return obs
