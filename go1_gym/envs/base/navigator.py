...
from go1_gym.envs.base.nav_base_task import BaseTask
from go1_gym.utils.math_utils import quat_apply_yaw, wrap_to_pi, get_scale_shift, get_euler_yaw
from go1_gym.utils.terrain import Terrain
from .navigator_config import Cfg
...
class LeggedRobot(BaseTask):
    def __init__(self, cfg: Cfg, sim_params, physics_engine, sim_device, headless, eval_cfg=None,
        ...
        self.load_loco_policy() # phase2

    # phase2
    def load_loco_policy(self):

        obs_history_length = self.cfg.env.num_observation_history
        num_obs_history = obs_history_length * self.num_obs
        self.loco_obs_history = torch.zeros(self.num_envs, num_obs_history, dtype=torch.float,
                                       device=self.device, requires_grad=False)

        self.loco_gaits = {"pronking": [0, 0, 0],
                "trotting": [0.5, 0, 0],
                "bounding": [0, 0.5, 0],
                "pacing": [0, 0, 0.5]}
        ...
        self.loco_body_height_cmd = 0.0
        self.loco_step_frequency_cmd = 3.0
        self.loco_gait = torch.tensor(self.loco_gaits["trotting"])
        self.loco_footswing_height_cmd = 0.08
        self.loco_pitch_cmd = 0.0
        self.loco_roll_cmd = 0.0
        self.loco_stance_width_cmd = 0.25
    ...
    def step(self, nav_actions):
        ...
        """
        clip_nav_actions_xy = self.cfg.normalization.clip_nav_actions_xy
        self.nav_actions = torch.clip(nav_actions, -clip_nav_actions_xy, clip_nav_actions_xy).to(self.device)
        clip_nav_actions_yaw = self.cfg.normalization.clip_nav_actions_yaw
        self.nav_actions[:, 2] = torch.clip(nav_actions[:, 2], -clip_nav_actions_yaw, clip_nav_actions_yaw).to(self.device)
        # self.nav_actions[:, :2] = nav_actions[:, :2]*-2
        # self.nav_actions[:, 2] = nav_actions[:, 2]*-0.6

        # self.nav_actions = nav_actions.to(self.device)
        
        with torch.no_grad():
            loco_actions = self.loco_policy(self.loco_obs_history)

        self.commands[:, :3] = self.nav_actions[:, :3]
        # self.commands[:, 0] = 3.0
        # self.commands[:, 1] = 0.0
        # self.commands[:, 2] = 0.0

        self.commands[:, 3] = self.loco_body_height_cmd
        self.commands[:, 4] = self.loco_step_frequency_cmd
        self.commands[:, 5:8] = self.loco_gait
        self.commands[:, 8] = 0.5
        self.commands[:, 9] = self.loco_footswing_height_cmd
        self.commands[:, 10] = self.loco_pitch_cmd
        self.commands[:, 11] = self.loco_roll_cmd
        self.commands[:, 12] = self.loco_stance_width_cmd

        return self.loco_step(loco_actions)
    
    def loco_step(self, actions):
        """ Apply actions, simulate, call self.post_physics_step()
        ...
        self.nav_obs_buf = torch.clip(self.nav_obs_buf, -clip_obs, clip_obs) # phase2

        # print("action", self.nav_actions[0])
        # print(self.nav_actions)
    ...
    def post_physics_step(self):
        ...
        # prepare quantities
        self.base_pos[:] = self.root_states[self.robot_actor_idxs][:self.num_envs, 0:3]
        self.base_quat[:] = self.root_states[self.robot_actor_idxs][:self.num_envs, 3:7]
        self.base_lin_vel[:] = quat_rotate_inverse(self.base_quat, self.root_states[self.robot_actor_idxs][:self.num_envs, 7:10])
        self.base_ang_vel[:] = quat_rotate_inverse(self.base_quat, self.root_states[self.robot_actor_idxs][:self.num_envs, 10:13])
        ...
        self.precompute_requirements() # phase2
        ...
        self.last_nav_actions[:] = self.nav_actions[:] # phase2
    ...
    def check_termination(self):
        ...
        """
        # TODO check termination via contacts on bodies
        ...
        self.reset_buf |= self.task_completed_buf # phase2
    ...
    # 1 post_physics_step, 1 BaseTask.reset
    def reset_idx(self, env_ids):
        ...
        # reset buffers
        self.last_nav_actions[env_ids] = 0. # phase2
    ...
    def set_idx_pose(self, env_ids, dof_pos, base_state):
        ...
        robot_env_ids = self.robot_actor_idxs[env_ids].to(self.device)
        robot_env_ids_int32 = robot_env_ids.to(dtype=torch.int32).to(self.device)
    ...
    # phase2
    def precompute_requirements(self):
        self.distance_to_target = self.env_targets - self.base_pos


        robot_yaw = normalize_angle(get_euler_yaw(self.base_quat))
        target_yaw = torch.atan2(self.distance_to_target[:, 1], self.distance_to_target[:, 0])
        # target_yaw = robot_yaw*0 # make zero to point towards x

        self.yaw_diff_to_target = torch.abs(target_yaw - robot_yaw)

        # self.yaw_diff_to_target = get_yaw_diff(self.base_quat, self.base_init_state[3:7])
        # x_is_done = torch.abs(self.distance_to_target[:, 0]) < self.cfg.rewards.pos_threshold
        # y_is_done = torch.abs(self.distance_to_target[:, 1]) < self.cfg.rewards.pos_threshold
        # yaw_is_done = torch.abs(self.yaw_diff_to_target) < self.cfg.rewards.yaw_threshold

        self.task_completed_buf = self.distance_to_target[:, 0] < 0.01
    ...
    def compute_observations(self):
        ...
        """
        # phase2
        self.nav_obs_buf = torch.cat((self.base_pos[:, :2] - self.env_origins[:, :2],
                                      self.base_quat,
                                      self.base_lin_vel[:, :2],
                                      self.base_ang_vel[:, 2:3],
                                      self.nav_actions,
                                      self.last_nav_actions,
                                      self.env_targets[:, :2] - self.env_origins[:, :2],
                                      self.box_obs
                                      ), dim=-1)
        
        # self.nav_obs_buf = torch.cat((self.base_pos,
        #                               self.base_quat,
        #                             #   self.nav_actions,
        #                             #   self.last_nav_actions,
        #                               ), dim=-1)
    ...
    def create_sim(self):
        ...
        # self.gym.toggle_visualization_overlay(self.sim, "grid", True)
    ...
    # ------------- Callbacks --------------
    # 5 reset_idx, 5 _post_physics_step_callback, 2 _create_envs
    ...
    # 2 _post_physics_step_callback, 1 _create_envs
    ...
    # 1 reset_idx, 2 _post_physics_step_callback, 1 _create_envs
    ...
    # 1 reset_idx, 1 _post_physics_step_callback
    ...
    def _reset_root_states(self, env_ids, cfg):
        ...
        self.root_states[robot_env_ids, :3] += self.env_origins[env_ids]
        self.root_states[robot_env_ids, 2] = self.base_init_state[2]

        # init_yaws = torch_rand_float(3.14, 3.14, (len(robot_env_ids), 1), device=self.device)
        init_yaws = torch.zeros(len(robot_env_ids), 1, device=self.device, dtype=torch.float)
        quat = quat_from_angle_axis(init_yaws, torch.Tensor([0, 0, 1]).to(self.device))[:, 0, :]
        self.root_states[robot_env_ids, 3:7] = quat
        self.root_states[robot_env_ids, 7:13] = torch_rand_float(-0, 0, (len(robot_env_ids), 6),
                                                           device=self.device)
        
        # randomise
        # self.root_states[robot_env_ids, 0:1] += torch_rand_float(0.0, 0.8, (len(robot_env_ids), 1), device=self.device)
        # self.root_states[robot_env_ids, 1:2] += torch_rand_float(-0.5, 0.5, (len(robot_env_ids), 1), device=self.device)
        ...
        if cfg.env.record_video and 0 in robot_env_ids:
        ...
        if cfg.env.record_video and self.eval_cfg is not None and self.num_train_envs in robot_env_ids:
    ...
    def _init_buffers(self):
        ...
        self.nav_actions = torch.zeros(self.num_envs, self.num_nav_actions, dtype=torch.float, device=self.device,
                                   requires_grad=False) # phase2
        self.last_nav_actions = torch.zeros(self.num_envs, self.num_nav_actions, dtype=torch.float, device=self.device,
                                        requires_grad=False) # phase2
    ...
    def _create_envs(self):
        ...
        wall_asset_1 = self.gym.create_box(self.sim, 5.0, 0.1, 1, wall_asset_options)
        wall_asset_2 = self.gym.create_box(self.sim, 0.1, 2.0, 1, wall_asset_options)
        ...
        for name in self.cfg.asset.terminate_after_contacts_on: # TODO
        ...
        self.env_targets[:,0] += self.cfg.init_state.target_pos_shift[0]
        self.env_targets[:,1] += self.cfg.init_state.target_pos_shift[1]
        ...
        # phase2
        self.env_targets += self.env_origins[:]
        ...
        # phase3
        self.box_x_offsets = torch.FloatTensor(self.num_envs).uniform_(1, 2.3).to(self.device)
        self.box_y_offsets = torch.FloatTensor(self.num_envs).uniform_(-0.5, 0.5).to(self.device)
        self.box_lengths = torch.FloatTensor(self.num_envs).uniform_(0.3, 1).to(self.device)
        self.box_widths = torch.FloatTensor(self.num_envs).uniform_(0.4, 0.4).to(self.device)

        self.box_obs = torch.cat((self.box_x_offsets.unsqueeze(1), self.box_y_offsets.unsqueeze(1),
                                  self.box_lengths.unsqueeze(1), self.box_widths.unsqueeze(1)), dim=1)
        ...
        for i in range(self.num_envs):
            ...
            start_pose.p = gymapi.Vec3(pos[0]+1.5, pos[1]+1, 0)
            wall_handle_1 = self.gym.create_actor(env_handle, wall_asset_1, start_pose, "wall1", i, 0)
            start_pose.p = gymapi.Vec3(pos[0]+1.5, pos[1]-1, 0)
            wall_handle_2 = self.gym.create_actor(env_handle, wall_asset_1, start_pose, "wall2", i, 0)
            start_pose.p = gymapi.Vec3(pos[0]+4, pos[1], 0)
            wall_handle_3 = self.gym.create_actor(env_handle, wall_asset_2, start_pose, "wall3", i, 0)
            start_pose.p = gymapi.Vec3(pos[0]-1, pos[1], 0)
            wall_handle_4 = self.gym.create_actor(env_handle, wall_asset_2, start_pose, "wall4", i, 0)
            
            self.wall_handles.append(wall_handle_1)
            self.wall_handles.append(wall_handle_2)
            self.wall_handles.append(wall_handle_3)
            self.wall_handles.append(wall_handle_4)

            self.wall_actor_idxs.append(self.gym.get_actor_index(env_handle, wall_handle_1, gymapi.DOMAIN_SIM))
            self.wall_actor_idxs.append(self.gym.get_actor_index(env_handle, wall_handle_2, gymapi.DOMAIN_SIM))
            self.wall_actor_idxs.append(self.gym.get_actor_index(env_handle, wall_handle_3, gymapi.DOMAIN_SIM))
            self.wall_actor_idxs.append(self.gym.get_actor_index(env_handle, wall_handle_4, gymapi.DOMAIN_SIM))
        
            # phase3
            box_asset = self.gym.create_box(self.sim, self.box_widths[i], self.box_lengths[i], 1, wall_asset_options)

            start_pose.p = gymapi.Vec3(pos[0]+self.box_x_offsets[i], pos[1]+self.box_y_offsets[i], 0)
            box_handle = self.gym.create_actor(env_handle, box_asset, start_pose, "box", i, 0)
            self.gym.set_rigid_body_color(env_handle, box_handle, 0, gymapi.MESH_VISUAL, gymapi.Vec3(1, 0, 0))
    ...
    def _render_headless(self):
        ...
        if self.record_eval_now and self.complete_video_frames_eval is not None and len(
            ...
            if self.eval_cfg is not None:
                bx, by, bz = self.root_states[self.robot_actor_idxs][self.num_train_envs, 0], self.root_states[self.robot_actor_idxs][self.num_train_envs, 1], \
                             self.root_states[self.robot_actor_idxs][self.num_train_envs, 2]
                self.gym.set_camera_location(self.rendering_camera_eval, self.envs[self.num_train_envs],
    ...
    def _get_env_origins(self, env_ids, cfg):
        ...
        if cfg.terrain.mesh_type in ["heightfield", "trimesh"]:
            ...
            xx = xx.to(self.device) # phase2
            yy = yy.to(self.device) # phase2
    ...
    def _parse_cfg(self, cfg):
        ...
        # max_episode_length_s = cfg.env.episode_length_s # phase2
        # cfg.env.max_episode_length = np.ceil(max_episode_length_s / self.dt) # phase2
