...
class Runner:
    ...
    def __init__(self, env, device='cpu'):
        ...
        # phase2
        actor_critic = ActorCritic(self.env.num_nav_obs,
                                    #   self.env.num_privileged_obs,
                                      self.env.num_nav_obs_history,
                                      self.env.num_nav_actions,
                                      ).to(self.device)
        ...
        # phase2
        # self.alg.init_storage(self.env.num_train_envs, self.num_steps_per_env, [self.env.num_nav_obs],
        #                       [self.env.num_privileged_obs], [self.env.num_nav_obs_history], [self.env.num_nav_actions])
        self.alg.init_storage(self.env.num_train_envs, self.num_steps_per_env, [self.env.num_nav_obs],
                               [self.env.num_nav_obs_history], [self.env.num_nav_actions])
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            with torch.inference_mode():
                for i in range(self.num_steps_per_env):
                    ...
                    actions_train = self.alg.act(obs[:num_train_envs], obs_history[:num_train_envs])
                    if eval_expert:
                    ...
                    done_count = (dones==1).sum().item() # phase 3
            ...
            if logger.every(RunnerArgs.log_freq, "iteration", start_on=1):
                ...
                logger.log_metrics_summary(key_values={"timesteps": self.tot_timesteps, "iterations": it, 'success': done_count})
                logger.job_running()
